...
client.on("interactionCreate", async (interaction) => {
  ...
  console.log(
    `Time: ${new Date().toLocaleString(Intl.DateTimeFormat().resolvedOptions().locale)} / Request User ID: ${interaction.user.id} / Type: ${interaction.type}`
  );
  ...
  let banned = false;
  await requestToAPI(requestData)
    .then((response) => {
